...
test.describe('BLK-PRECOS (4.5.2)', () => {
  ...
  test('PRC-01 PRC-02 PRC-03 manual entry lifecycle and sync routine', async ({ page }) => {
    const materialName = buildQaTitle('QA preco manual')
    const regionName = 'SP'
    ...
    await page.goto('/admin/precos')
    const syncOverviewCard = page.locator('div', { hasText: /sincronização automática|sincronizacao automatica/i }).first()
    await expect(syncOverviewCard).toBeVisible()
    await page.getByRole('button', { name: /sincronizar agora/i }).click()
    await expect(
      page.getByText(/sincronização concluída|sincronizacao concluida|não foi possível sincronizar|nao foi possivel sincronizar/i),
