        ...
        <>
          ...
          <AdminDataTable
            columns={[
              ...
              { header: 'Conteúdo', cell: (row) => <div aria-label={`Abrir mensagem ${row.title}`} className="cursor-pointer space-y-2 rounded-xl border border-transparent p-2 text-left transition hover:border-border hover:bg-muted/30" onClick={() => setSelectedHistoryItem(row)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedHistoryItem(row) } }} role="button" tabIndex={0}><p className="font-medium text-foreground line-clamp-1">{row.title}</p><p className="text-xs text-muted-foreground">Categoria: {row.category}</p><p className="text-xs font-medium text-foreground">Destinatário(s): <span className="font-normal text-muted-foreground">{row.recipientSummary}</span></p><p className="text-xs font-semibold text-primary">Abrir mensagem</p></div> },
