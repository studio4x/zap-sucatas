          ...
          <div className="space-y-6 p-6">
            ...
            <div className="grid gap-3">
              <Link className="rounded-[1.4rem] border border-border bg-background px-5 py-4 text-left transition hover:bg-muted" onClick={() => onOpenChange(false)} to={paths.public.support}>
                ...
                <p className="mt-1 text-sm leading-6 text-muted-foreground">Veja respostas rápidas para dúvidas comuns.</p>
              </Link>
              <button className="rounded-[1.4rem] border border-primary/20 bg-primary/5 px-5 py-4 text-left transition hover:bg-primary/10" onClick={() => setStep('form')} type="button">
                ...
                <p className="mt-1 text-sm leading-6 text-muted-foreground">Escreva o que aconteceu e, se quiser, envie um anexo.</p>
              </button>
            </div>
          ...
          <div className="max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <h2 className="mt-1 text-2xl font-semibold tracking-tight text-foreground">Abrir chamado</h2>
                    <p className="mt-2 text-sm leading-6 text-muted-foreground">Descreva o problema com suas palavras para agilizar o atendimento.</p>
                  </div>
                </div>
            ...
            <div className="space-y-5 p-6">
              <div className="rounded-[1.4rem] border border-primary/10 bg-primary/5 p-4">
                <p className="text-sm font-semibold text-foreground">Prazo estimado para a primeira resposta</p>
                ...
                <p className="mt-2 text-xs text-muted-foreground">A prioridade ajuda na organização interna e não muda o prazo prometido ao cliente.</p>
              </div>

              {!user?.profileId ? (
                <div className="rounded-[1.4rem] border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                  Faça login para abrir um chamado e acompanhar o histórico de atendimento.
                  <div className="mt-3">
              ...
              <form className="space-y-4" onSubmit={form.handleSubmit((values) => createMutation.mutate(values))}>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground" htmlFor="support-priority">Urgência do caso</label>
                  ...
                  {user?.role !== 'admin' ? (
                    <p className="inline-flex items-center gap-2 text-xs text-muted-foreground"><Lock className="size-3.5" /> Essa informação ajuda a equipe a organizar os atendimentos.</p>
                  ) : null}
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-medium text-foreground" htmlFor="support-description">Descrição</label>
                  <Textarea id="support-description" placeholder="Explique o que aconteceu, o que você já tentou e o que precisa resolver agora." {...form.register('description')} />
                ...
                <div className="flex justify-between gap-3 pt-2">
                  ...
                  <Button disabled={!user?.profileId || createMutation.isPending} type="submit">{createMutation.isPending ? 'Enviando...' : 'Enviar mensagem'}</Button>
                </div>
              </form>
            </div>
          </div>
