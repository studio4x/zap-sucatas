...
export const notificationBroadcastSchema = z.object({
  ...
  body: z.string().trim().min(3, 'Informe a mensagem.').max(3000, 'A mensagem deve ter no maximo 3000 caracteres.'),
  category: z.string().trim().min(2, 'Informe a categoria.').max(80, 'A categoria deve ter no maximo 80 caracteres.'),
  ...
  title: z.string().trim().min(3, 'Informe o titulo.').max(200, 'O titulo deve ter no maximo 200 caracteres.'),
